...
import ThemeToggelBtn from "./ThemeToggelBtn";
...
const Navbar = ({theme, setTheme}) => {
  ...
  return (
    <div className="flex items-center justify-between px-4 sm:px-12 lg:px-24 xl:px-40 py-4 sticky top-0 z-20 backdrop:blur-xl font-medium bg-white/50 dark:bg-gray-900/70">
        ...
        {/* navlinks */}
        <div className={`text-gray-700 dark:text-white sm:text-sm ${!sidebaropne ? 'max-sm:w-0 overflow-hidden' : 'max-sm:w-60 max-sm:pl-10'} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-primary max-sm:text-white max-sm:pt-20 sm:text-center flex gap-5 transition-all `}>
          ...
          />
            ...
            <a onClick={() => setSideBarOpen(false)} href="#our-work" className="sm:hover:border-b">Our Works</a>
            <a onClick={() => setSideBarOpen(false)} href="#contact-us" className="sm:hover:border-b">Contact Us</a>
        </div>

        <div className="flex items-center gap-2 sm:gap-4">

          <ThemeToggelBtn theme={theme} setTheme={setTheme}/>

          <img src={theme === "dark" ? assets.menu_icon_dark : assets.menu_icon} alt="" 
          onClick={ () => setSideBarOpen(true)} className="w-8 sm:hidden"
          />

          <a href="#contact-us" className="text-sm max-sm:hidden flex items-center gap-2 bg-primary text-white px-6 py-2 rounded-full cursor-pointer hover:scale-105 transition-all">Connect <img src={assets.arrow_icon} width={14} alt="" />
          </a>
        </div>
    </div>
  );
